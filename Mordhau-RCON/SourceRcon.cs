...
namespace Mordhau_RCON
{
	/// <summary>
	/// SourceRcon Class
	/// Copyright(c) 2008-2011, Andrew Simpson
	/// (modified by WaGi-Coding aka. Taki7o7 - 2021)
	/// </summary>
	public class SourceRcon
	{
		...
		void SendRCONPacket(RCONPacket p, string cmd)
		{
			byte[] Packet = p.OutputAsBytes();
			if (S.Connected)
			{
				S.BeginSend(Packet, 0, Packet.Length, SocketFlags.None, new AsyncCallback(SendCallback2), cmd);
			}
		}
		...
		bool firstAliveDone = false;
		void SendCallback2(IAsyncResult ar)
		{
            if (!firstAliveDone)
            {
				firstAliveDone = true;
			...
			OnCommandOutput((string)ar.AsyncState);
		...
		void ReceiveCallback(IAsyncResult ar)
		{
			...
			{
				//disconnect request, catiching mainly accessing disposed socket
		...
		internal void OnDisconnected(string output)
		{
			if (Disconnect != null)
			{
				Disconnect(output);
		...
		internal void OnSuccessOutput(string output)
		{
		...
		internal void OnCommandOutput(string output)
		{
		...
		public event StringOutput Disconnect;
		public event StringOutput SuccessOutput;
		public event StringOutput CommandOutput;
		...
		public static string ConnectionTimeout = "Connection Timeout!";
		public static string WrongPassword = "Wrong Password! Kicking...";
		public static string UnknownConnectionIssue = "Cannot connect! Unknown reason.";
